...
namespace SR {
	namespace DSP {
		namespace Airwindows {
			// DSP class from airwindows Tube2
			class Tube2 {
			public:
				enum EParam {
					kParamA = 0,
				...
				*/
				...
				double Tube2::GetParameterNormalized(EParam index) {
					switch (index) {
			...
			inline void Tube2::ProcessBlock(double** inputs, double** outputs, int sampleFrames) {
				...
				while (--sampleFrames >= 0)
				{
					...
					} //for high sample rates on this Tube2 we are going to do a simple average		
