...
def  main():
  ...
  installer = "\installer\SRDynamicsControl Installer.exe"
   
  if demo:
    installer = "\installer\SRDynamicsControl Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SRDynamicsControl manual.pdf" 
  ]
  ...
  ZIPNAME = "SRDynamicsControl-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SRDynamicsControl-v" + FULLVERSIONSTR + "-win-demo.zip"
